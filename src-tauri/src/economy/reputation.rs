...
impl ReputationEngine {
    ...
    pub fn add_review(&mut self, agent_id: &str, rating: f64, comment: String, reviewer_id: String) -> Review {
        let review = Review {
            reviewer_id,
    ...
    fn recalculate(&mut self, agent_id: &str) {
        let agent_reviews: Vec<&Review> = self.reviews.iter()
            .filter(|(id, _)| id == agent_id)
            .map(|(_, r)| r)
            .collect();

        if agent_reviews.is_empty() { return; }

        let avg_rating = agent_reviews.iter().map(|r| r.rating).sum::<f64>() / agent_reviews.len() as f64;
        ...
        let bonus = if success_rate > 0.98 { 0.1 } else if success_rate > 0.95 { 0.05 } else { 0.0 };
        let score = (avg_rating + bonus).clamp(1.0, 5.0);

        let mut badges = Vec::new();
        if tasks_completed >= 1000 { badges.push("1000+ tasks".to_string()); }
        if success_rate >= 0.98 { badges.push("98%+ success".to_string()); }
        if review_count >= 100 { badges.push("100+ reviews".to_string()); }
