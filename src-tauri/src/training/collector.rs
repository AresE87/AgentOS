...
impl TrainingCollector {
    ...
    pub fn get_summary(conn: &Connection) -> Result<TrainingSummary, String> {
        ...
        let total: i64 = conn.query_row("SELECT COUNT(*) FROM training_records", [], |r| r.get(0)).unwrap_or(0);
        let successful: i64 = conn.query_row("SELECT COUNT(*) FROM training_records WHERE success = 1", [], |r| r.get(0)).unwrap_or(0);
        let with_feedback: i64 = conn.query_row("SELECT COUNT(*) FROM training_records WHERE feedback_rating IS NOT NULL", [], |r| r.get(0)).unwrap_or(0);
        let positive: i64 = conn.query_row("SELECT COUNT(*) FROM training_records WHERE feedback_rating > 0", [], |r| r.get(0)).unwrap_or(0);
